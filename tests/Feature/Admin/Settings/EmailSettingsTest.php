...
it('saves resend settings', function () {
    Volt::test('admin.settings.email')
        ->set('mailer', 'resend')
        ->set('resendKey', 're_123456789')
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('email-settings-saved');

    expect(settings('email.mailer'))->toBe('resend');
    expect(settings('email.resend_key'))->toBe('re_123456789');
});

it('saves brevo settings', function () {
    Volt::test('admin.settings.email')
        ->set('mailer', 'brevo')
        ->set('brevoKey', 'xkeysib-123456789')
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('email-settings-saved');

    expect(settings('email.mailer'))->toBe('brevo');
    expect(settings('email.brevo_key'))->toBe('xkeysib-123456789');
});
