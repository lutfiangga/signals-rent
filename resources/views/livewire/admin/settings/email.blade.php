...
new #[Layout('components.layouts.app')] #[Title('Email')] class extends Component {
    ...
    public function mount(): void
    {
        ...
        $this->resendKey = (string) $group['resend_key'];
        $this->brevoKey = (string) $group['brevo_key'];
    ...
    public function save(): void
    {
        ...
        $validated = $this->validate([
            ...
            'resendKey' => $rules['resend_key'],
            'brevoKey' => $rules['brevo_key'],
        ...
        settings()->setMany([
            ...
            'email.resend_key' => ['value' => $validated['resendKey'], 'type' => $types['resend_key'] ?? 'string'],
            'email.brevo_key' => ['value' => $validated['brevoKey'], 'type' => $types['brevo_key'] ?? 'string'],
    ...
    <x-admin.layout group="preferences" title="Email" description="Configure how your application sends email.">
        <x-signals.form-section title="Mail Provider">
            <form wire:submit="save" class="space-y-6">
                ...
                {{-- Resend Settings --}}
                <div x-show="$wire.mailer === 'resend'" x-cloak class="space-y-4">
                    <flux:input wire:model="resendKey" label="Resend API Key" type="password" />
                </div>

                {{-- Brevo Settings --}}
                <div x-show="$wire.mailer === 'brevo'" x-cloak class="space-y-4">
                    <flux:input wire:model="brevoKey" label="Brevo API Key" type="password" />
                </div>
